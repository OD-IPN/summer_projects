...
if(isset($_GET["v"]) && isset($_GET["id"])){
	if($_GET["v"]=="MLB"){
		$infoSede = consultaCursoIdMLB($_GET['id'], $conexion);
...
?>
<h2>Grupo <?php echo $infoSede[0]->nombre; ?></h2>
<table class="[ reporte ]"> 
	<tbody>
		<tr>
		...
		$inscritos = json_decode(consultaRegistradosGrupoMLB($infoSede[0]->id, $conexion));
		$sinPagar = json_decode(consultaHorariosEstadoMLB('0', $infoSede[0]->id, $conexion));
		$validando = json_decode(consultaHorariosEstadoMLB('1', $infoSede[0]->id, $conexion));
		$pagado = json_decode(consultaHorariosEstadoMLB('2', $infoSede[0]->id, $conexion));
		$rechazados = json_decode(consultaHorariosEstadoMLB('3', $infoSede[0]->id, $conexion));
		...
		<tr>
			<td><center><?php echo($infoSede[0]->dia_1.'<br/>'.$infoSede[0]->hr_1); ?></center></td>
			<td><center><?php echo($infoSede[0]->dia_2.'<br/>'.$infoSede[0]->hr_2); ?></center></td>
			<td><center><?php echo($infoSede[0]->dia_3.'<br/>'.$infoSede[0]->hr_3); ?></center></td>
		</tr>
	</tbody>
</table>
<table class="[ table table-striped ]">
	<tbody>
		...
		<tr>
			<td> <center> <?php echo($inscritos[0]->integrantes); ?> </center> </td>
...
<table class="[ table table-striped ]">
	<tbody>
		...
		<?php 
		$infoXsede = json_decode(consultaInscripcionIdMLB($infoSede[0]->id, $conexion));
		if($infoXsede)
		...
		<tr>
			...
			<?php $estadoPago = json_decode(consultaEstadoUserCursoMLB($infoXsede[$f]->id_usuario, $_GET['id'], $conexion));?>
